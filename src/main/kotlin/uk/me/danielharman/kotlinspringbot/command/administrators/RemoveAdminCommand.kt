    ...
    override fun execute(event: MessageReceivedEvent) {
        ...
        when (administratorService.getBotAdministratorByDiscordId(event.author.id)) {
            ...
            is Success -> {
                ...
                if (split.size < 2) {
                    event.channel
                        .sendMessageEmbeds(Embeds.createErrorEmbed("No enough parameters supplied"))
                        .queue()
                    return
                }
                ...
                val removeAdministrator = administratorService.removeBotAdministrator(user.id)

                if (removeAdministrator is Failure) {
                    event.channel
                        .sendMessageEmbeds(Embeds.createErrorEmbed(removeAdministrator.reason))
                        .queue()
                    return
                }
