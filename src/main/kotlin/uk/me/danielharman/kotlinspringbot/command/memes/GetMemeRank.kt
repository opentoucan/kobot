    ...
    override fun execute(event: DiscordMessageEvent) {
        ...
        memerIds.forEach { pair ->
            try {
                ...
                val name =
                    member?.nickname
                        ?: when (val user = discordService.getUserById(pair.first)) {
                            is Failure -> "Unknown"
        ...
        event.reply(
            EmbedBuilder()
                .setTitle(
                    if (asc) "Losers in ${event.guild.name}"
                    else "Meme ranking for ${event.guild.name}")
                .setColor(Color.GRAY)
                .appendDescription(des.toString())
                .build())
    }
}
